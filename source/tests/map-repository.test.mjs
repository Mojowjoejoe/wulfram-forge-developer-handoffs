...
void test('checkout privacy policy blocks publishing before branch, index, commit, or remote changes', (context) => {
  const { repository, remote } = realRepository(context);
  const changed = fixtureProject();
  changed.terrain.heights[4] = 44.5;
  saveRepositoryMap(repository, 'git-diff-test', changed, { scope: 'terrain' });
  git(repository, 'config', '--local', 'wulfram.publishDisabled', 'true');
  const diagnostics = repositoryDiagnostics(repository);
  const policy = diagnostics.checks.find((check) => check.id === 'publication-policy');
  assert.equal(policy.status, 'warn');
  assert.match(policy.detail, /Publish is blocked/);
  assert.equal(diagnostics.checks.find((check) => check.id === 'origin').fix, '');
  const before = {
    head: git(repository, 'rev-parse', 'HEAD'),
    branch: git(repository, 'branch', '--show-current'),
    diff: git(repository, 'diff'),
    index: git(repository, 'diff', '--cached'),
    remote: git(repository, 'ls-remote', '--heads', remote),
  };
  assert.throws(() => publishRepositoryMaps(repository, ['git-diff-test'], {
    publishBranch: () => assert.fail('private checkout must never invoke publisher'),
  }), /disabled by this checkout privacy policy/);
  assert.deepEqual({
    head: git(repository, 'rev-parse', 'HEAD'),
    branch: git(repository, 'branch', '--show-current'),
    diff: git(repository, 'diff'),
  ...
  git(repository, 'config', '--local', 'wulfram.publishDisabled', 'invalid-boolean');
  assert.throws(() => publishRepositoryMaps(repository, ['git-diff-test'], {
    publishBranch: () => assert.fail('invalid privacy policy must fail closed'),
  }), /bad boolean|invalid boolean/i);
  assert.equal(git(repository, 'rev-parse', 'HEAD'), before.head);
  assert.equal(git(repository, 'diff', '--cached'), before.index);
});
