...
const resultOut=await fs.mkdtemp(path.join(prior.out,'courtyard-visual-matrix-'));
...
try{
 ...
 for(const [index,c] of matrix.cases.entries()){
  ...
  const source=JSON.parse(bytes);source.name=`Courtyard visual ${index} ${c.size}`;source.terrain.tagmap=['0:1snow001'];source.terrain.tagmap2=['1snow001'];
 ...
 report.passed=true;
}catch(error){report.error=error.stack;process.exitCode=1;}finally{if(socket)socket.close();app.kill();await fs.writeFile(path.join(resultOut,'courtyard-visual-report.json'),JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));}
