...
assert.equal(prior.passed,true);assert.equal(prior.courtyardPortable.reuseApplyUndo,true);
assert.equal(createHash('sha256').update(await fs.readFile(prior.executable)).digest('hex'),prior.executableSha256);
const resultOut=await fs.mkdtemp(path.join(prior.out,'courtyard-restart-'));
...
process.env.WULFRAM_MCP_SESSION_DIR=path.join(prior.out,'courtyard-restart-sessions');
...
try{
 ...
 const exported=JSON.parse(await fs.readFile((prior.brokenRingPortable??prior.courtyardPortable).exportPath,'utf8'));assert.deepEqual(favorites,exported.bases);const matching=favorites.filter(f=>f.reservations?.family==='service-courtyard');assert.equal(matching.length,1);const favorite=matching[0];
 const fixture=JSON.parse(await fs.readFile(path.resolve(process.env.WULFRAM_COURTYARD_TEST_MAP),'utf8'));fixture.name='Courtyard restart destination';fixture.terrain.worldWidth=16000;fixture.terrain.worldHeight=12000;fixture.terrain.tagmap=['0:1snow001'];fixture.terrain.tagmap2=['1snow001'];
 const file=path.join(resultOut,'courtyard-restart-destination.json');await fs.writeFile(file,JSON.stringify(fixture));
 ...
 const areas=JSON.parse(layout.metadata['forge.build-areas.v1']);assert.equal(areas.length,6);
 favorite.reservations.areas.forEach((a,i)=>{const actual=areas[i],x=a.side===1?4000:12000,y=6000,sign=a.side===1?1:-1;assert.equal(actual.id,a.id);assert.equal(actual.width,a.width);a.points.forEach((p,j)=>p.forEach((n,k)=>assert.ok(Math.abs(actual.points[j][k]-((k===0?x:y)+sign*n))<1e-6)));});
 for(const team of [1,2]){const entities=layout.entities.filter(e=>e.team===team);assert.equal(entities.length,favorite.template.units.length);entities.forEach((e,i)=>{const u=favorite.template.units[i],sign=team===1?1:-1;assert.equal(e.token,u.token);assert.equal(e.active,u.active);assert.ok(Math.abs(e.position[0]-((team===1?4000:12000)+sign*u.offset[0]))<1e-5);assert.ok(Math.abs(e.position[1]-(6000+sign*u.offset[1]))<1e-5);assert.ok(Math.abs(Math.atan2(Math.sin(e.rotation[2]-(u.rotation[2]+(team===2?Math.PI:0))),Math.cos(e.rotation[2]-(u.rotation[2]+(team===2?Math.PI:0)))))<1e-5);});}
 const access=JSON.parse(layout.metadata['formation.courtyardAccess']);assert.equal(access.vehicleWidth,80);assert.deepEqual(access.routes.map(r=>r.team),[1,2]);for(const [i,r] of access.routes.entries()){const [court,west,east]=areas.slice(i*3,i*3+3);assert.deepEqual(r.points,[west.points[0],west.points[1],court.points[1],east.points[1]]);assert.ok(!r.markers.some(m=>m.severity==='blocked'));}
 const saved=path.join(resultOut,'courtyard-restart-applied.json');await fs.writeFile(saved,JSON.stringify(applied));
 await click('Inspect');
 const screenshots=[];
 ...
 report.persistedExactLibrary=true;report.crossMapDimensions=[16000,12000];report.previewMapHistoryUnchanged=true;report.transformedGeometry=true;report.destinationAccess=true;report.applyUndo=true;report.appliedCopy=saved;report.passed=true;
}catch(error){report.error=error.stack;process.exitCode=1;}finally{if(socket)socket.close();app.kill();await fs.writeFile(path.join(resultOut,'courtyard-restart-report.json'),JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));}
