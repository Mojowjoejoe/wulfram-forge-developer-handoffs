...
export function repositoryDiagnostics(repository) {
  ...
  if (fs.existsSync(path.join(resolved, '.git')) && gitVersion.ok) {
    try {
      ...
      const publishDisabled = runGit(resolved, ['config', '--type=bool', '--default=false', '--get', 'wulfram.publishDisabled']) === 'true';
      checks.push(diagnostic(
        'publication-policy',
      ...
      checks.push(diagnostic(
        ...
        publishDisabled ? '' : `git remote set-url origin https://github.com/${MAPS_REPOSITORY_NAME}.git`,
      ));
...
export function publishRepositoryMaps(repository, slugs, options = {}) {
  const resolved = assertMapsRepository(repository);
  if (runGit(resolved, ['config', '--type=bool', '--default=false', '--get', 'wulfram.publishDisabled']) === 'true') {
    throw new Error('Publishing is disabled by this checkout privacy policy. Save locally; use an explicitly verified private remote outside the editor.');
  }
  const selected = slugs?.length
