...
async function main() {
  ...
  const expectedSuffix = process.env.WEBVIEW2_FIXED_RUNTIME_DIR ? 'offline-fixed-webview2' : 'self-contained';
  const existingArtifact = path.join(workspace, 'dist', 'desktop', `WulframForge-${version}-win-x64-${expectedSuffix}.zip`);
  if (fs.existsSync(existingArtifact)) throw new Error(`Release already exists; choose a new version to preserve it: ${existingArtifact}`);
