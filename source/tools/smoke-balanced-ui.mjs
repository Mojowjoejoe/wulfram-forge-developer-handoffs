...
await evaluate(`(() => {
  const input = document.querySelector('input[name=balanced-name]');
...
await evaluate(`(() => {
  ...
  buttons.find((button) => button.textContent.includes('Keep seed & regenerate'))?.click();
  buttons[0]?.click();
})()`);
await waitFor("document.querySelector('.balanced-generator-dialog') === null");
...
const randomizedSeed = await evaluate("document.querySelector('input[name=balanced-seed]').value");
...
await evaluate(`(() => {
  const input = document.querySelector('input[name=balanced-seed]');
  Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set.call(input, 'forge-001');
  input.dispatchEvent(new Event('input', { bubbles: true }));
})()`);
const generationStartedAt = performance.now();
await evaluate("Array.from(document.querySelectorAll('.balanced-generator-footer button')).find((button) => button.textContent.includes('Keep seed & regenerate'))?.click()");
