...
internal sealed class MapRepositoryHost
{
        ...
        try
        {
            ...
            object result = action switch
            {
                ...
                "app-info" => new {
                    ...
                    runtimeVersion = webView.Environment.BrowserVersionString,
                },
    ...
    {
        string root = RequireRepository();
        if (RunGit(root, "config", "--type=bool", "--default=false", "--get", "wulfram.publishDisabled") == "true")
            throw new InvalidOperationException("Publishing is disabled by this checkout privacy policy. Save locally; use an explicitly verified private remote outside the editor.");
        _ = MapDirectory(slug);
    ...
    private object Diagnostics()
    {
        ...
        if (checkoutExists && hasGit)
        {
            try
            {
                ...
                bool publishDisabled = RunGit(resolved, "config", "--type=bool", "--default=false", "--get", "wulfram.publishDisabled") == "true";
                checks.Add(Check(
                    "publication-policy",
                    "Publication privacy policy",
                    publishDisabled ? "warn" : "pass",
                    publishDisabled
                        ? "Publish is blocked by checkout policy. Local saves remain available; use an explicitly verified private remote outside the editor."
                        : "No checkout publication block is configured. Publish targets the official public maps repository."));
                ...
                checks.Add(Check(
                    ...
                    publishDisabled ? "" : $"git remote set-url origin https://github.com/{RepositoryName}.git"));
