...
import { parseTerrainTextureTag } from './terrain-textures.ts';
import {
...
export function balancedSeedHash(seed: string): number {
  return hashSeedText(normalizeSeed(seed));
}

// Internal domain separators are not user seed characters. Validate the public
// seed before composition, but allow the composed hash input to exceed 200.
function hashSeedText(seed: string): number {
  let hash = 0x811c9dc5;
  for (const byte of new TextEncoder().encode(seed.normalize('NFC').trim())) {
    hash ^= byte;
...
/** Randomize draft controls only; fairness validation still runs at generation. */
export function randomizeBalancedSettings(seed: string, templateIds: string[], textureNames: string[]) {
  const normalizedSeed = normalizeSeed(seed);
  if (!templateIds.length || !textureNames.length) throw new Error('Available starter bases and terrain themes are required.');
  const random = seededRandom(`${normalizedSeed}\0settings-v1`);
  ...
  return {
    seed: normalizedSeed,
    topology: pick<BalancedMapTopology>(['open-field', 'three-route', 'ring-center']),
    baseSeparation: integer(30, 65) / 100,
    routeWidth: integer(10, 35) / 20,
    centralAreaSize: integer(5, 25) / 10,
    size: pick([33, 65, 129, 257]),
    worldWidth: integer(32, 80) * 100,
    worldHeight: integer(32, 80) * 100,
    relief: integer(10, 120) * 10,
    templateId: pick(templateIds),
    textureName: pick(textureNames),
  };
}
...
function basePadBlend(x: number, y: number, separation: number): number {
  const anchors: Array<[number, number]> = [[-separation, -separation], [separation, separation]];
...
export function routeBlend(topology: BalancedMapTopology, x: number, y: number, separation: number, width: number): number {
  const alongAxis = (x + y) / Math.SQRT2;
  const crossAxis = (y - x) / Math.SQRT2;
  const baseAxis = separation * Math.SQRT2;
  ...
  return smoothstep(0.055 * width, (topology === 'three-route' ? 0.4 : 0.29) * width, distance);
}
...
export function generateBalancedTerrain(options: BalancedTerrainOptions): BalancedTerrainResult {
  ...
  const baseSeparation = finiteOption(options.baseSeparation, 0.46, 'Base separation');
  const routeWidth = finiteOption(options.routeWidth, 1, 'Route width');
  const centralAreaSize = finiteOption(options.centralAreaSize, 1, 'Central area size');
  if (baseSeparation < 0.3 || baseSeparation > 0.65) throw new Error('Base separation must be from 0.30 to 0.65 of the map diagonal.');
  if (routeWidth < 0.5 || routeWidth > 1.75) throw new Error('Route width multiplier must be from 0.5 to 1.75.');
  if (centralAreaSize < 0.5 || centralAreaSize > 2.5) throw new Error('Central area size multiplier must be from 0.5 to 2.5.');
  ...
  const heights = shapedHeights.map((terrainHeight, index) => {
    ...
    const flattened = baseHeight + (routed - baseHeight) * basePadBlend(normalizedX, normalizedY, baseSeparation);
    const objectiveBlend = smoothstep(0.055 * centralAreaSize, 0.105 * centralAreaSize, Math.hypot(normalizedX, normalizedY));
  ...
  return {
    ...
    terrain,
    baseAnchors: [
      [worldWidth * (0.5 - baseSeparation / 2), worldHeight * (0.5 - baseSeparation / 2)],
      [worldWidth * (0.5 + baseSeparation / 2), worldHeight * (0.5 + baseSeparation / 2)],
    ],
...
export function rotationalTerrainMismatches(terrain: TerrainData, tolerance = 0): number {
  ...
  for (let index = 0; index < expected; index += 1) {
    ...
    if (!Number.isFinite(terrain.heights[index]) || Math.abs(terrain.heights[index] - terrain.heights[pair]) > tolerance) mismatches += 1;
  }
  // Textures are packed CELLS, not height vertices. Compare actual layers with
  // their corner masks rotated, not numeric tag IDs or the unused trailing slots.
  const cells = (terrain.width - 1) * (terrain.height - 1);
  const canonical = (id: number, rotate: boolean) => parseTerrainTextureTag(terrain.tagmap2[id])
    .map(({ name, corners }) => `${name}:${rotate
      ? ((corners & 1) << 3) | ((corners & 2) << 1) | ((corners & 4) >> 1) | ((corners & 8) >> 3)
      : corners}`).sort().join('|');
  for (let index = 0; index < cells; index += 1) {
    const first = canonical(terrain.textureIds[index], false);
    if (!first || first !== canonical(terrain.textureIds[cells - 1 - index], true)) mismatches += 1;
  }
  return mismatches;
}
...
function deterministicIdFactory(seed: string, team: number): (prefix: string) => string {
  const hash = hashSeedText(`${BALANCED_GENERATOR_VERSION}\0${seed}`).toString(16).padStart(8, '0');
  let sequence = 0;
...
export function generateBalancedProject(
  ...
  const generatorMetadata = {
    ...
    'generator.parameters': JSON.stringify({
      ...
      baseSeparation: generated.identity.baseSeparation,
      routeWidth: generated.identity.routeWidth,
      centralAreaSize: generated.identity.centralAreaSize,
