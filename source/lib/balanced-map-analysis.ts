...
export function analyzeRotationalEntityPairs(project: WulframProject, manifest?: AssetManifest): EntityPairingAnalysis {
  ...
  const comparePair = (left: StateEntity, right: StateEntity, expectedTeam: number) => {
    ...
    // Team art can have different local origins. Compare model-bottom elevation,
    // not raw origin Z; absent bounds retain the historical raw-height check.
    const leftBounds = structureTerrainClearance(left, manifest, 0, 0);
    const rightBounds = structureTerrainClearance(right, manifest, 0, 0);
    const hasBothBounds = leftBounds.modelName && rightBounds.modelName;
    const heightDelta = Math.abs((right.position[2] - (hasBothBounds ? rightBounds.modelBottom : 0))
      - (left.position[2] - (hasBothBounds ? leftBounds.modelBottom : 0)));
  ...
  return {
    ...
    message: passed
      ? `${matchedPairs} team/neutral entity pairs satisfy the rotational transform.${manifest ? ' Height comparison uses model-bottom elevations where both model bounds exist.' : ''}`
...
function analyzeTeam(
  ...
  objectiveRegions: Array<Set<number>>,
  highGroundThreshold: number | null,
  profile: BalancedTraversalProfile,
  ...
  if (source === undefined) {
    return {
      ...
      objectiveCost: null,
      objectiveCosts: objectiveRegions.map(() => null),
      routeCount: 0,
      alternateObjectiveCost: null,
      reachableHighGroundFraction: 0,
      reachableObjectiveCount: 0,
    };
  }
  ...
  const objectiveCosts = objectiveRegions.map((region) => {
    const route = objectiveRegions.length === 1 ? first : shortestPath(terrain, passable, source, region);
    return route && Number.isFinite(route.cost) ? route.cost : null;
  });
  ...
  return {
    ...
    reachableObjectiveCount: objectiveRegions.filter((region) => [...region].some((index) => reached[index])).length,
  };
}
...
export function analyzeBalancedTerrain(
  ...
  const sampledClearanceRadiusWorld: [number, number] | null = shapeValid
    ? [
      profile.minimumRouteClearanceVertices * terrain.worldWidth / (terrain.width - 1),
      profile.minimumRouteClearanceVertices * terrain.worldHeight / (terrain.height - 1),
    ]
    : null;
  ...
  const objectiveRegions = objectiveAnchorsValid
    ? objectiveAnchors.map((anchor) => targetVertices(terrain, clearancePassable, [anchor], profile.objectiveRadiusVertices))
    : [];
  const objectives = new Set(objectiveRegions.flatMap((region) => [...region]));
  ...
  const regionCostDeltas = objectiveAnchorsValid ? objectiveAnchors.map((anchor, index) => {
    const partner = objectiveAnchors.findIndex((candidate) => Math.hypot(
      candidate[0] - (terrain.worldWidth - anchor[0]),
      candidate[1] - (terrain.worldHeight - anchor[1]),
    ) <= 1e-6);
    const left = teams[0].objectiveCosts[index];
    const right = teams[1].objectiveCosts[partner];
    return left != null && right != null
      ? Math.abs(left - right) / Math.max(1, left, right)
      : null;
  }) : [];
  const maximumPairedRegionCostDeltaRatio = regionCostDeltas.length > 0
    && regionCostDeltas.every((delta) => delta !== null)
    ? regionCostDeltas.reduce<number>((maximum, delta) => Math.max(maximum, delta!), 0)
    : null;
  const gates: BalanceGate[] = [
    ...
    {
      code: 'objectives',
      passed: objectiveAnchorsValid && objectiveRegions.length > 0
        && teams.every((team) => team.reachableObjectiveCount === objectiveRegions.length),
      message: objectiveAnchorsValid && objectiveRegions.length > 0
        && teams.every((team) => team.reachableObjectiveCount === objectiveRegions.length)
        ? `Both teams can reach all ${objectiveRegions.length} required objective regions.`
        : `Every required objective must be valid and reachable by both teams; reached ${teams.map((team) => team.reachableObjectiveCount).join('/')} of ${objectiveAnchors.length} regions.`,
    },
    {
      code: 'route-count',
      passed: teams.every((team) => team.routeCount >= profile.minimumRouteCount),
      message: `Detected ${teams[0].routeCount}/${teams[1].routeCount} separated team routes after a ${profile.minimumRouteClearanceVertices}-vertex clearance proxy; minimum ${profile.minimumRouteCount} each. ${sampledClearanceRadiusWorld
        ? `Sampled clearance radius: X ${sampledClearanceRadiusWorld[0].toFixed(3)}, Y ${sampledClearanceRadiusWorld[1].toFixed(3)} world units; not verified vehicle clearance.`
        : 'Physical clearance extent unavailable for invalid terrain.'}`,
    },
    {
    ...
    },
    {
      ...
      message: pairedObjectiveCostDeltaRatio === null
        ? 'Nearest objective cost cannot be compared.'
        : `Nearest objective-cost delta is ${(pairedObjectiveCostDeltaRatio * 100).toFixed(9)}%.`,
    },
    {
      code: 'paired-region-cost',
      passed: maximumPairedRegionCostDeltaRatio !== null
        && maximumPairedRegionCostDeltaRatio <= profile.pairedCostTolerance,
      message: maximumPairedRegionCostDeltaRatio === null
        ? 'Every rotational objective-region pair must have reachable costs for comparison.'
        : `Maximum rotational objective-region cost delta is ${(maximumPairedRegionCostDeltaRatio * 100).toFixed(9)}%; offline path-cost proxy, not gameplay timing.`,
    },
  ];
  return {
    ...
    metrics: {
      ...
      sampledClearanceRadiusWorld,
      highGroundThreshold,
      pairedObjectiveCostDelta,
      pairedObjectiveCostDeltaRatio,
      maximumPairedRegionCostDeltaRatio,
      teams,
    },
  };
}
...
export function analyzeBalancedProject(
  ...
  const entityPairing = analyzeRotationalEntityPairs(project, manifest);
