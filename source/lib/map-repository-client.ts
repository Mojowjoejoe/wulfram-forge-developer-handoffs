...
export interface EditorBuildInfo {version:string;runtimeVersion:string}
export async function getEditorBuildInfo():Promise<EditorBuildInfo|undefined>{
  if(!hasNativeRepositoryBridge())return undefined;
  const info=await nativeRequest<EditorBuildInfo>('app-info',{},5000);
  if(!info||typeof info.version!=='string'||typeof info.runtimeVersion!=='string')throw new Error('The native host returned invalid build information.');
  return info;
}
