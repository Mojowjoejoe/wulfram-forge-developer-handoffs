...
# We use max_total=16 bits and a two-bit precision selector.  Two selector
# bits encode four levels (0..3), giving 13..16 data bits.  The client computes
# base_bits as max_total_bits - (1 << header_bits) + 1.

# --- 2. DYNAMIC VECTOR CONFIG (Position) ---
# Logic: "I want between 13 and 16 bits, controlled by a 2-bit header."
# Setup: head=2, total=16.
# Math: selector levels = 2^2 = 4; base = 16 - 4 + 1 = 13.
# Result: Header 0=13 bits, Header 3=16 bits.
VEC_POS  = {"head": 2, "total": 16, "max": "8192.0", "range": "16384.0"} # +/- 8192

# +/- 200 *should* enough for tank velocity
VEC_VEL  = {"head": 2, "total": 16, "max": "1000.0",  "range": "2000.0"}  # +/- 1000

# Increased to cover 2*PI (approx 6.28). 
# Range 12.6 allows for -6.3 to +6.3, covering full rotation safely.
VEC_ROT  = {"head": 2, "total": 16, "max": "6.3",    "range": "12.6"}    # +/- 2*PI

# Angular velocity of 10 rads/sec should be fast enough (~1.5 full spins/sec)
VEC_SPIN = {"head": 2, "total": 16, "max": "200.0",   "range": "400.0"}   # +/- 10.0
