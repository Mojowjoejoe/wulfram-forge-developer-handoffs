...
class WorldStatsPacket(Packet):
    ...
    [Type 0x16]
    [String] Map Name
    [Byte]   Strategic-map column count
    [Byte]   Strategic-map row count
    [4 Bytes] Signed 16.16 value converted to float
    """
    SPACE_MAP_COLUMNS = 6
    SPACE_MAP_ROWS = 6
    ...
    def serialize(self) -> bytes:
        pkt = PacketWriter()
        pkt.write_string(self.map_name)
        pkt.write_byte(self.SPACE_MAP_COLUMNS)
        pkt.write_byte(self.SPACE_MAP_ROWS)
